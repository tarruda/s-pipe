...
function getStateId(items) {
  var rv = [];

  for (var i = 0, l = items.length; i < l; i++) {
    var item = items[i];
    rv.push(item.ruleId + ',' + item.pos);
  }
...
var State = classic({
  constructor: function State(grammar, it) {
  ...
  follow: function(states, grammar) {
    ...
    for (symbol in possibleInputs) {
      ...
      nextState = new State(grammar, possibleInputs[symbol]);

      if (!has(states, nextState.id)) {
        states[nextState.id] = nextState;
