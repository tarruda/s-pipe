...
function calculateFollowSet(grammar, ruleId) {
  var rv = {};
    ...
    for (var i = 0, l = grammar.rules.length; i < l; i++) {
      ...
      for (var j = 0, l2 = rule.rhs.length; j < l2; j++) {
        ...
        if (component === nonTerminal) {
          if (rule.rhs.length === j + 1) continue;
  ...
  for (i = 0, l = itemSet.length; i < l; i++) {
    ...
    if (item.rhs.length > item.pos) {
      ...
      if (!action) {
        ...
        var followSet = calculateFollowSet(grammar, action.by);
        // Create a new state just for the reduce rule
        var reduceItemSet = [reduceItem];
        var reduceState = buildState(grammar, dfa, reduceItemSet);
        // Plus a new state containing all item sets in this state minus
        // the item responsible for the above reduce 
        var shiftItemSet = _(itemSet)
                            (reject, function(item) {
                              return item === reduceItem;
                            })();
        var shiftState = buildState(grammar, dfa, shiftItemSet);
        // This state's action will become 'lookahead' and select one of the
        // above states
        state.action = {
          type: 'lookahead',
          followSet: followSet,
          consequent: reduceState.id,
          alternate: shiftState.id
        };
...
var ParseStream = PullStream.extend({
  ...
  _processToken: function(token, chunk) {
    var stack = this._parseStack, dfa = this.dfa, states = dfa.states, err;
    var rules = dfa.rules, state = stack.topState;

    if (state.action.type === 'lookahead') {
      var action = state.action;
      stack.popStates(1);

      if (has(action.followSet, token))
