...
function calculateShiftSet(grammar, shiftItemSet) {
  var ruleIds = [];
  var rv = {};

  for (var i = 0, l = shiftItemSet.length; i < l; i++) {
    var item = shiftItemSet[i];
  ...
  //   if (has(processed, ruleId)) continue;
    ...
    for (var i = 0, l = grammar.rules.length; i < l; i++) {
      ...
      for (var j = 0, l2 = rule.rhs.length; j < l2; j++) {
        ...
        if (component === nonTerminal) {
          if (rule.rhs.length === j + 1 && !has(processed, rule.lhs)) {
            // The nonterminal is the last component of this rule, so anything
            // that can follow this rule can also follow the nonterminal
            nonTerminals.push(rule.lhs);
            continue;
          }
          var next = rule.rhs[j + 1];
          var set = calculateShiftSet(grammar, [{rhs: [next], pos: 0}]);
          rv = extend(rv, set);
          break;
        }
      }
...
function findReductionPath(grammar, dfa, kernel) {
  function find(state) {
    if (state.action && state.action.type === 'reduce')
      return;
    if (!state.transitions || Object.keys(state.transitions).length === 1)
      return;
    var rv = [];
    for (var k in state.transitions) {
      if (!has(state.transitions, k)) continue;
      var next = dfa.states[state.transitions[k]];
      var path = [];
      if (!has(grammar.ruleIds, k)) {
        path.push(k);
      }
      rv.push(path.concat(find(next)));
    }
    return rv;
  }
...
function lookahead(grammar, dfa, reduceItem, itemSet, state) {
  ...
  // // Get the set of tokens that the top of the lookahead/pending buffer
  // // should be checked against before proceeding to the 'reduce' state
  // for (var k in reduceSet) {
  //   if (!has(reduceSet, k)) continue;
  //   if (!has(grammar.ruleIds, k)) continue;
  //   var ruleIds = grammar.ruleIds[k];
  //   var kernel = [];
  //   for (var i = 0, l = ruleIds.length; i < l; i++) {
  //     var ruleId = ruleIds[i];
  //     var rule = grammar.rules[ruleId];
  //     kernel.push({ruleId: ruleId, lhs: rule.lhs, rhs: rule.rhs, pos: 0});
  //   }
  //   var reductionPath = findReductionPath(grammar, dfa, kernel);
  ...
  for (i = 0, l = itemSet.length; i < l; i++) {
    ...
    if (item.rhs.length > item.pos) {
      ...
      if (!action) {
        ...
        return lookahead(grammar, dfa, reduceItem, itemSet, state);
      }
      ...
      if (action) {
        if (action.type !== 'reduce') {
          ...
          return lookahead(grammar, dfa, item, itemSet, state);
        }
      } else {
...
var ParseStream = PullStream.extend({
  ...
  _processToken: function(token, chunk) {
    ...
    if (action.type === 'lookahead') {
      stack.popStates(1);

      if (has(action.shiftSet, token)) {
        state = states[action.consequent];
      } else if (has(action.reduceSet, token)) {
        // the token is in the reduce follow set, so it must be queued
        // for after reduction
        pending.push([token, chunk]);
    ...
    while (state && state.action.type === 'reduce') {
      ...
      var actionName = dfa.symbols[rule.action];
      var count = rule.rhs.length;
      var popped = stack.popSymbols(count);
      stack.popStates(count);
      var parsed = _(popped)(map, function(p) { return p.value; })();
      if (has(actions, actionName)) {
        var actionResult = actions[actionName].apply(this, parsed);
        if (!is.nil(actionResult))
          parsed = actionResult;
      } else if (parsed.length === 1) {
        parsed = parsed[0];
      }
      if (rule.lhs === -1) {
        // Accept, push everything except the helper EOF token
  ...
  _processPending: function() {
    var pending = this._parseStack.pending;

    while (pending.length) {
      var p = pending.shift();
      this._processToken(p[0], p[1]);
    }
  },

  _process: function(chunk) {
    var stack = this._parseStack, pending = stack.pending;

    this._processPending();
    this._processToken(this.dfa.symbolIds[this._getToken(chunk)], chunk);

    if (stack.topState && this._done()) {
