...
var ParseStack = classic({
  constructor: function ParseStack() {
    this.symbolStack = [];
    this.stateStack = [];
    this.topSymbol = null;
    this.topState = null;
...
var ParseStream = PullStream.extend({
  constructor: function ParseStream(source, grammar, factory) {
    ...
    this._parseStack = new ParseStack();
  },

  _process: function(chunk) {
    var stack = this._parseStack, table = this.table;
    var state = stack.topState || table[0];
    ...
    if (this._done()) {
      ...
      var result = stack.popSymbols(1)[0].value;

      if (stack.topState || stack.topSymbol) {
        this._emit('error', new Error('Unexpected end of input'));
      } else {
        this.push(result);
      }
    }
