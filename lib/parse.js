...
function expandGrammar(grammar) {
  var rules = [], ruleIds = {}, symbols = ['EOF'], symbolIds = {EOF: 0};
  var rv = {
  ...
  rules.unshift({
    id: 0,
    lhs: null,
    rhs: [getSymbolId(startRule, symbols, symbolIds)]
  });
...
function closeItemSet(grammar, kernel) {
  ...
  function addItem(item) {
    ...
    // remain 
    if (symbolId && has(grammar.ruleIds, symbolId)) {
...
function buildState(grammar, dfa, kernel) {
  ...
  var nextKernels = {};
  var state = {id: null, action: null}, action, i, l;
  ...
  for (i = 0, l = itemSet.length; i < l; i++) {
    ...
    if (item.rhs.length > item.pos) {
      ...
      if (!action)
        ...
        // TODO solve conflict with lookahead
...
var ParseStream = PullStream.extend({
  ...
  _process: function(chunk) {
    ...
    var symbolIds = dfa.symbolIds, rules = dfa.rules, state = stack.topState;

    var token = symbolIds[this._getToken(chunk)];
