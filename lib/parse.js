...
var State = classic({
  constructor: function State(ruleId, pos) {
    this.ruleId = ruleId;
    this.id = ruleId + '[' + pos + ']';
    this.pos = pos;
    this.items = [];
  ...
  follow: function(states, grammar) {
    ...
    if (has(grammar.rules, this.ruleId)) {
      ...
      for (i = 0, l = items.length; i < l; i++) {
        item = items[i];
        this.addItem(states, grammar, item);
      }
    }
  ...
  addItem: function(states, grammar, item) {
    if (!is.array(item)) item = [item];
    ...
    if (has(grammar.rules, currentSymbol)) {
      var currentSymbolItem = grammar.rules[currentSymbol];

      // Recursiverly add each item produced by the rule
      for (var i = 0, l = currentSymbolItem.length; i < l; i++) {
        this.addItem(states, grammar, currentSymbolItem[i]);
      }
    }
  }
});
...
function createTable(grammar) {
  ...
  var states = {};
  var initialState = new State(start, 0);
